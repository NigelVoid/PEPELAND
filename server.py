...
@app.route('/writeToFileFriend/<name>/<friend>', methods=['POST'])
def write_to_fileFriend(name, friend):
    data = [str(i) for i in os.listdir('static/chats') if name in i and friend in i]
    text = request.json['text']
    if str(text).strip() != '':
        if '!/!' in text:
            text = str(text).replace('!/!', ' ')
        with open(f'static/chats/{data[0]}', 'a') as file:
            file.write('!/!' + f'{name}: {text}')
...
def addFriendProfile(name, name2):
    with open(f'static/user/{name}/profile.json', 'r', encoding='utf8') as file:
        fileData = json.load(file)
        if name2 not in fileData['Friends']:
    ...
    with open(f'static/user/{name2}/profile.json', 'r', encoding='utf8') as file:
        fileData = json.load(file)
        if name not in fileData['Friends']:
    ...
    with open(f'static/user/{name}/invites.json', 'r', encoding='utf8') as file:
        fileData = json.load(file)
        fileData['invites'] = [str(i) for i in fileData['invites'] if str(i) != name2]
    ...
    with open(f'static/user/{name2}/invites.json', 'r', encoding='utf8') as file:
        fileData = json.load(file)
        fileData['invites'] = [str(i) for i in fileData['invites'] if str(i) != name]

    with open(f'static/user/{name2}/invites.json', 'w', encoding='utf8') as file:
        json.dump(fileData, file)

    with open(f'static/chats/{name}+{name2}.txt', 'w') as file:
        pass
...
@app.route('/chatWithFriend/<friend>/<name>')
def chatWithFriend(name, friend):
    with open(f'static/user/{name.strip()}/identification.json', 'r',
              encoding='utf8') as file:
        fileData = json.load(file)
        if fileData['ip'] != f'{session["token"]}':
            ...
            return render_template('chatWithFriend.html', name=name, friend=friend)


@app.route('/chat/<name>/<friend>')
def get_textChatWithFriend(name, friend):
    data = [str(i) for i in os.listdir('static/chats') if name in i and friend in i]

    def generate(previous_state=[None]):
        while True:
            current_state = os.stat(f'static/chats/{data[0]}').st_mtime
            if previous_state[0] is None or current_state != previous_state[0]:
                with open(f'static/chats/{data[0]}', 'r') as file:
                    yield 'data: ' + file.read() + '\n\n'
                previous_state[0] = current_state
...
def otherprofile(name, name2, id, number, portal):
    if request.method == 'GET':
        ...
        photos = [f'/static/user/{name2}/photos/{photo}' for photo in os.listdir(f'static/user/{name2}/photos')]

        videos = os.listdir(f'static/user/{name2}/video')

        with open(f'static/user/{name2}/profile.json', 'r', encoding='utf8') as file:
            fileDataprofile = json.load(file)

        endata = []
        entry_files = [f for f in os.listdir(f'static/user/{name2}/entrys') if f.endswith('.json')]
        for entry_file in entry_files:
            with open(f'static/user/{name2}/entrys/{entry_file}', 'r', encoding='utf8') as file:
...
@app.route('/PEPELAND/API')
def api():
    response = requests.get("https://catfact.ninja/fact")
    fact = f'{response.json()["fact"]}'
    response = requests.get("https://official-joke-api.appspot.com/random_joke")
    setup = f"{response.json()['setup']}"
    punchline = f"{response.json()['punchline']}"
    response = requests.get("http://universities.hipolabs.com/search?country=Kazakhstan")
    universities = response.json()
    univ = [i['name'] for i in universities]
    return render_template('PEPELAND.html', text_cat=fact, joke1=setup,
                           joke2=punchline, univ=univ)
...
def profile(name, id, number, portal):
    if request.method == 'GET':
        ...
        path_user = f'static/user/{name.strip()}'
        identification_file = f'{path_user}/identification.json'

        with open(identification_file, 'r', encoding='utf8') as file:
            file_data = json.load(file)
            if file_data['ip'] != f'{token}':
                return abort(403)

        avatar_path = f'/{path_user}/avatar/{os.listdir(f"{path_user}/avatar")[0]}'
        photos = [f'/static/user/{name}/photos/{photo}' for photo in os.listdir(f"{path_user}/photos")]
        videos = [video for video in os.listdir(f"{path_user}/video")]

        profile_data_path = f'static/user/{name}/profile.json'
        with open(profile_data_path, 'r', encoding='utf8') as file:
        ...
        if file_data['id'] == id and file_data['number'] == number and file_data['portal'] == portal:
            return render_template('profile.html', name=name, path_photos=photos,
                                   Friends=profile_data['Friends'],
                                   id=id,
                                   number=number, portal=portal, path=avatar_path, path_videos=videos,
                                   endata=endata, members=os.listdir('static/user'))
